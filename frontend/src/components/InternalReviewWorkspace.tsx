...
const isReviewScope = (value: string | null): value is ReviewScope =>
  value === "pending" || value === "all" || value === "closed";
...
const REVIEW_WORKFLOW_STEPS = [
  {
    key: "submitted",
    label: "Submitted",
    description: "Application is in the internal queue.",
  },
  {
    key: "initial_review",
    label: "Initial Review",
    description: "Operations verifies the core business file.",
  },
  {
    key: "document_check",
    label: "Document Check",
    description: "Required documents are accepted or flagged.",
  },
  {
    key: "compliance_review",
    label: "Compliance",
    description: "Final internal checks confirm readiness.",
  },
  {
    key: "decision",
    label: "Decision",
    description: "Application is approved or declined.",
  },
] as const;

const getWorkflowProgressIndex = (status: string): number => {
...
const getWorkflowStepState = (
  ...
  const activeIndex = getWorkflowProgressIndex(currentStatus);
...
function InternalReviewWorkspace(): JSX.Element {
  ...
  const [scope, setScope] = useState<ReviewScope>(
    isReviewScope(initialScopeParam) ? initialScopeParam : "pending",
  );
  ...
  const filteredQueue = useMemo(() => {
    ...
    nextItems.sort((left, right) => {
      ...
      const rightTime = new Date(
        right.submittedAt || right.updatedAt,
      ).getTime();
  ...
  useEffect(() => {
    ...
    if (selectedApplicationId) {
      nextParams.set("applicationId", selectedApplicationId);
    }

    setSearchParams(nextParams, {
      replace: true,
    });
  }, [scope, selectedApplicationId, setSearchParams]);
  ...
  const selectedApplicationSummary = useMemo(() => {
    ...
    const pendingSections =
      selectedApplication.sections.length - completedSections;
    const latestTask =
  ...
  const selectedApplicationView = useMemo(() => {
    ...
    return {
      ...
      banking: isPayer
        ...
        : selectedApplication.agentOperations ||
          selectedApplication.merchantBanking,
      declaration: isPayer
        ? selectedApplication.payerDeclaration
        : selectedApplication.agentDeclaration ||
          selectedApplication.merchantDeclaration,
      outlets: isAgent
        ? selectedApplication.agentOperations?.outlets || []
        : [],
  ...
  const commentSummary = useMemo(() => {
    ...
    return {
      ...
      unresolved: selectedApplication.comments.filter(
  ...
  const handleAction = async (action: ReviewAction): Promise<boolean> => {
    ...
    if (
      ...
      setError(
        "A review note is required for request info and reject actions.",
      );
      return false;
    }

    if (action === "approve" && !canApproveApplication) {
      setError("Approve is locked until every required document is accepted.");
      return false;
    }
  ...
  const handleCreateComment = async (): Promise<void> => {
    ...
    try {
      ...
      setCommentSectionKey("");
      setCommentType(
        commentVisibility === "internal" ? "general" : "correction",
      );
  ...
  return (
    <section className="review-workspace">
      ...
      >
        <article className="review-summary__card" role="listitem">
        ...
        <aside className="review-queue">
          <div className="review-queue__header">
            <div>
              ...
              <p className="review-queue__copy">
                Submitted files, correction loops, and completed decisions.
              </p>
            </div>
          ...
          {!loadingQueue && queue.length === 0 ? (
            <div className="empty-state">
              ...
              <span>
                Try a different filter or submit an application first.
              </span>
            </div>
          ) : null}
                ...
                return (
                  ...
                  >
                    <div className="review-queue__item-top">
                      <div>
                        ...
                        <p>
                          ...
                          {humanize(item.organization.entityType)}
                        </p>
                      </div>
                      <span
                        className={`status-badge status-badge--${getApplicationStatusVariant(
                          item.status,
                        )}`}
                      >
                    ...
                    <div className="review-queue__meta">
                      <span>Started by {item.startedBy.fullName}</span>
                      <span>
                        {item.sectionProgress.completed}/
                        {item.sectionProgress.total} sections
                      </span>
                    </div>
                    ...
                    <div className="review-queue__progress">
                      ...
                      <div className="review-queue__progress-meta">
        ...
        <div className="review-detail">
          {!selectedApplicationId ? (
            <div className="empty-state">
              ...
              <span>
                The review detail will appear here once an item is selected.
              </span>
            </div>
          ) : loadingDetail ? (
            <div className="empty-state">
              ...
              <span>
                The selected application is being prepared for review.
              </span>
            </div>
          ) : selectedApplication ? (
            <>
              <section className="page-section review-record-hero">
                <div className="panel-header">
                  ...
                  <div className="panel-header__summary review-record-hero__summary">
                    <span
                      className={`status-badge status-badge--${getApplicationStatusVariant(
                        selectedApplication.status,
                      )}`}
                    ...
                    <span className="panel-header__meta">
                      Current step:{" "}
                      {humanize(
                        selectedApplication.currentStep || "not_started",
                      )}
                    </span>
                  </div>
                </div>
              ...
              <div className="review-case-layout">
                <div className="review-case-main">
                  <section className="form-section">
                    <div className="form-section__header">
                      <div>
                        ...
                        <p>
                          Business context, workflow position, and document
                    ...
                    <div className="workflow-rail review-workflow-rail">
                      {REVIEW_WORKFLOW_STEPS.map((step, index) => {
                        const stepState = getWorkflowStepState(
                          selectedApplication.status,
                          index,
                        );
                        ...
                        return (
                          <article
                            ...
                            className={`workflow-rail__item${
                              stepState === "complete"
                                ? " workflow-rail__item--complete"
                                : stepState === "current"
                                  ? " workflow-rail__item--current"
                                  : ""
                            }`}
                          >
                            <span
                              className="workflow-rail__dot"
                              aria-hidden="true"
                            />
                            <div>
                    ...
                    <div className="review-overview-grid">
                      <article className="review-overview-card">
                        ...
                        <p>
                          ...
                          {selectedApplication.sections.length} sections
                          complete
                        </p>
                      </article>
                      ...
                      <article className="review-overview-card">
                        ...
                        <strong>
                          {selectedApplication.uploadedDocuments.length} files
                        </strong>
                        <p>
                          {selectedApplicationSummary?.acceptedDocuments || 0}{" "}
                          accepted,{" "}
                          {selectedApplicationSummary?.flaggedDocuments || 0}{" "}
                          rejected
                        </p>
                      </article>
                        ...
                        <strong>
                          {formatValue(
                            selectedApplicationView?.primaryContact?.fullName ||
                              selectedApplication.businessSnapshot
                                ?.contactPerson ||
                              null,
                          )}
                        </strong>
                  ...
                  <section className="form-section">
                    <div className="form-section__header">
                      <div>
                        ...
                        <p>
                          Check completion across the current workflow before
                          taking a final action.
                        </p>
                      </div>
                      <span className="status-chip">
                        {selectedApplicationSummary?.pendingSections || 0}{" "}
                        section(s) pending
                      </span>
                    </div>
                      ...
                      {selectedApplication.sections.map((section) => (
                        ...
                        >
                          <div className="section-tracker__item-top">
                            ...
                            <span
                              className={getSectionStatusClassName(
                                section.status,
                              )}
                            >
                  ...
                  <div className="review-grid review-grid--detail">
                    <article className="review-card">
                      ...
                      <dl className="detail-list">
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplication.businessSnapshot?.legalName,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplication.businessSnapshot?.tradingName,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {humanize(
                              selectedApplication.organization.entityType,
                            )}
                          </dd>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.businessMetricValue,
                            )}
                          </dd>
                        </div>
                        <div>
                          <dt>
                            {selectedApplicationView?.serviceLabel ||
                              "Products and services"}
                          </dt>
                          <dd>
                            {formatValue(selectedApplicationView?.serviceValue)}
                          </dd>
                        </div>
                        {selectedApplicationView?.supplementalDetails.map(
                          (detail) => (
                    ...
                    <article className="review-card">
                      ...
                      <dl className="detail-list">
                        ...
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.primaryContact
                                ?.phoneNumber,
                            )}
                          </dd>
                        </div>
                        ...
                        <div>
                          <dt>
                            {selectedApplicationView?.principalsTitle ||
                              "Signatories"}
                          </dt>
                          <dd>
                            {selectedApplicationView?.principals.length || 0}
                          </dd>
                        </div>
                      </dl>
                    </article>
                    ...
                    <article className="review-card">
                      <h4>
                        {selectedApplicationView?.bankingTitle || "Banking"}
                      </h4>
                      <dl className="detail-list">
                        <div>
                          <dt>Account name</dt>
                          <dd>
                            {selectedApplicationView?.banking?.accountName ||
                              "-"}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.bankName,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.branchName,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.branchCode,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.accountNumber,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.accountType,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.banking?.currency,
                            )}
                          </dd>
                        </div>
                        {selectedApplicationView?.settlementExtras.map(
                          (detail) => (
                            <div key={detail.label}>
                              <dt>{detail.label}</dt>
                              <dd>{formatValue(detail.value)}</dd>
                    ...
                    <article className="review-card">
                      ...
                      <dl className="detail-list">
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.declaration?.signerName,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatValue(
                              selectedApplicationView?.declaration?.signerTitle,
                            )}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd>
                            {formatBoolean(
                              selectedApplicationView?.declaration
                                ?.acceptedTerms,
                            )}
                          </dd>
                        </div>
                    ...
                    <section className="form-section">
                      <div className="form-section__header">
                        <div>
                          ...
                          <p>
                            Everyone listed here can transact or act on behalf
                            of the{" "}
                            {selectedApplicationView?.applicationLabel ||
                              "application"}{" "}
                            during onboarding.
                          </p>
                        </div>
                      ...
                      {selectedApplicationView?.participants.length ? (
                              ...
                              >
                                ...
                                <dl className="detail-list detail-list--compact">
                                  ...
                                  <div>
                                    <dt>National ID</dt>
                                    <dd>
                                      {formatValue(person.nationalIdNumber)}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt>Residential address</dt>
                                    <dd>
                                      {formatValue(person.residentialAddress)}
                                    </dd>
                                  </div>
                                </dl>
                              </article>
                        ...
                        <div className="empty-state">
                          ...
                          <span>
                            This section was left empty in the submitted
                            application.
                          </span>
                        </div>
                      )}
                    ...
                    <section className="form-section">
                      <div className="form-section__header">
                        <div>
                          ...
                          <p>
                            ...
                            {selectedApplicationView?.principalDescriptor ||
                              "signatory"}{" "}
                            and whether a primary contact is clearly identified.
                          </p>
                        </div>
                        <span className="status-chip status-chip--soft">
                          {selectedApplicationView?.principals.length || 0}{" "}
                      ...
                      {selectedApplicationView?.principals.length ? (
                        ...
                        <div className="empty-state">
                          <strong>
                            No{" "}
                            {selectedApplicationView?.principalsTitle?.toLowerCase() ||
                              "signatories"}{" "}
                            listed
                          </strong>
                          <span>
                            The applicant did not provide any records in this
                            section.
                          </span>
                        </div>
                      )}
                    ...
                    <section className="form-section">
                      <div className="form-section__header">
                        <div>
                          ...
                          <p>
                            Review the branch and outlet footprint attached to
                      ...
                      {selectedApplicationView.outlets.length ? (
                        <div className="people-grid">
                          {selectedApplicationView.outlets.map(
                            (outlet, index) => (
                              <article
                                className="person-card"
                        ...
                        <div className="empty-state">
                          ...
                          <span>
                            The applicant did not provide any outlet records
                            here.
                          </span>
                        </div>
                      )}
                  ...
                  <section className="form-section">
                    <div className="form-section__header">
                      <div>
                        ...
                        <p>
                          Review file readiness at a glance before deciding on
                          the application.
                        </p>
                      </div>
                      <span className="status-chip status-chip--soft">
                    ...
                    <div
                      className={`document-completeness-banner${
                        canApproveApplication
                          ? ""
                          : " document-completeness-banner--warning"
                      }`}
                    >
                      <div>
                        <p className="panel-header__eyebrow">
                    ...
                    <div className="review-overview-grid">
                      ...
                      <article className="review-overview-card">
                        ...
                        <p>
                          Files still waiting for an internal document decision.
                        </p>
                      </article>
                      ...
                      <article className="review-overview-card">
                        ...
                        <p>
                          Required requirements that are still missing or
                          rejected.
                        </p>
                      </article>
                    </div>
                      ...
                      <article className="review-card">
                        ...
                        {selectedApplication.documentReviewSummary
                          ...
                          <div className="empty-state">
                            ...
                            <span>
                              The applicant has uploaded every required document
                              type.
                            </span>
                          </div>
                        )}
                      ...
                      <article className="review-card">
                        ...
                        {selectedApplication.documentReviewSummary
                            ...
                            {selectedApplication.documentReviewSummary.rejectedRequiredDocuments.map(
                              (item) => (
                                <li key={item.requirementCode}>
                                  <strong>{item.label}</strong>
                                  <span>
                                    {item.rejectedCount} rejected file(s) need
                                    replacement.
                                  </span>
                                </li>
                              ),
                          ...
                          <div className="empty-state">
                            ...
                            <span>
                              There are no rejected required documents at the
                              moment.
                            </span>
                          </div>
                        )}
                  ...
                  <section className="form-section">
                    ...
                    {selectedApplication.uploadedDocuments.length === 0 ? (
                      <div className="empty-state">
                        ...
                        <span>
                          This application does not have any uploaded files yet.
                        </span>
                      </div>
                    ) : (
                      <div className="document-grid">
                        {selectedApplication.uploadedDocuments.map(
                          (document) => (
                            <article
                              key={document.id}
                            ...
                            >
                              ...
                              <div className="document-card__meta">
                                <span>
                                  Requirement:{" "}
                                  {humanize(
                                    document.requirementCode || "unmapped",
                                  )}
                                </span>
                                <span>
                                  {document.isRequired
                    ...
                    <section className="form-section">
                      <div className="form-section__header">
                        <div>
                          ...
                          <p>
                            Track how this application moved through the
                            workflow.
                          </p>
                        </div>
                      </div>
                    ...
                    <section className="form-section">
                      <div className="form-section__header">
                        <div>
                          <h3>Review Tasks</h3>
                          <p>
                            Current and historical task items tied to this
                            application.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="task-list">
                        {selectedApplication.reviewTasks.length === 0 ? (
                          <div className="empty-state">
                            ...
                            <span>
                              Tasks will appear here once the workflow creates
                              them.
                            </span>
                          </div>
                        ) : (
                  ...
                  <section className="form-section">
                    <div className="form-section__header">
                      <div>
                        ...
                        <p>
                          Leave internal notes or applicant-visible correction
                          items tied to the exact section that needs attention.
                        </p>
                      </div>
                      <span className="status-chip status-chip--soft">
                    ...
                    <div className="review-overview-grid review-overview-grid--three">
                      <article className="review-overview-card">
                        <span className="review-overview-card__label">
                          Open Items
                        </span>
                        ...
                        <p>
                          Comments still waiting for follow-up or confirmation.
                        </p>
                      </article>
                      ...
                      <article className="review-overview-card">
                        ...
                        <strong>
                          {commentSummary?.unresolvedApplicant || 0}
                        </strong>
                        <p>
                          Outstanding notes the applicant can currently see.
                        </p>
                      </article>
                      ...
                      <article className="review-overview-card">
                        ...
                        <strong>
                          {commentSummary?.unresolvedInternal || 0}
                        </strong>
                        <p>
                          Reviewer-only notes kept inside the internal
                          workspace.
                        </p>
                      </article>
                    </div>
                      ...
                      <div className="form-grid">
                        ...
                        <label className="field">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setCommentSectionKey(event.target.value)
                            }
                            disabled={processingComment}
                          >
                ...
                <aside className="review-case-rail">
                  <section className="form-section review-actions-panel">
                    <div className="form-section__header">
                      <div>
                        <h3>Review Actions</h3>
                        <p>
                          Add a note, then request corrections, approve, or
                          reject the application. The request note will also be
                          sent back into the applicant-visible comment thread.
                        </p>
                      </div>
                      <span className="status-chip">
                        {isDecisionLocked
      ...
      {pendingDecision ? (
        ...
        >
          ...
          >
            ...
            {DECISION_ACTION_COPY[pendingDecision].requiresNote &&
            !reviewNote.trim() ? (
