...
const printUsage = (): void => {
  console.log(
    "Usage: npm run admin:create -- --name=\"Admin Name\" --email=\"admin@example.com\" --mobile=\"+263771234567\" --password=\"StrongPassword123\""
  );
};
...
const main = async (): Promise<void> => {
  ...
  console.log(
    `Admin user created: ${user.fullName} <${user.email}> (${user.mobileNumber})`
  );
};
